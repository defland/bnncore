        ...
        while (it != mapMasternodes.end()) {
            ...
            if (it->second.IsOutpointSpent()) {
                ...
                if(fAsk) {
                    ...
                    // wait for mnb recovery replies for MNB_RECOVERY_WBNC_SECONDS seconds
                    mMnbRecoveryRequests[hash] = std::make_pair(GetTime() + MNB_RECOVERY_WBNC_SECONDS, setRequested);
                }
                ++it;
            }
        }
