...
const char * const BITCOIN_CONF_FILENAME = "bluenode.conf";
const char * const BITCOIN_PID_FILENAME = "bluenoded.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bluenode" is a composite category enabling all Bluenode-related debug output
            if(ptrCategory->count(string("bluenode"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BluenodeCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BluenodeCore
    // Mac: ~/Library/Application Support/BluenodeCore
    // Unix: ~/.bluenodecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BluenodeCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BluenodeCore";
#else
    // Unix
    return pathRet / ".bluenodecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bluenode.conf if it does not excist
        FILE* configFile = fopen(GetConfigFile().string().c_str(), "a");
        if (configFile != NULL) {
            std::string strHeader = "# Default nodes config, the other config please put above this line\n"
                                    "addnode=node-01.nodes.blue:30503\n"
                                    "addnode=node-02.nodes.blue:30503\n"
                                    "addnode=node-03.nodes.blue:30503\n"
                                    "addnode=node-04.nodes.blue:30503\n"
                                    "addnode=node-05.nodes.blue:30503\n"
                                    "addnode=node-06.nodes.blue:30503\n"
                                    "addnode=node-07.nodes.blue:30503\n"
                                    "addnode=node-08.nodes.blue:30503\n"
                                    "addnode=node-09.nodes.blue:30503";
    ...
    {
        // Don't overwrite existing settings so command line settings override bluenode.conf
