                                 ...
                                 std::vector<std::pair<CMempoolAddressDeltaKey, CMempoolAddressDelta> > &results)
{
    ...
    for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
        addressDeltaMap::iterator BNN = mapAddress.lower_bound(CMempoolAddressDeltaKey((*it).second, (*it).first));
        while (BNN != mapAddress.end() && (*BNN).first.addressBytes == (*it).first && (*BNN).first.type == (*it).second) {
            results.push_back(*BNN);
            BNN++;
        }
    }
    return true;
